...
- **`herdr status server` exits 0 whether or not a server is running, so only the `status:` line answers that.** A crashed server leaves its socket files behind and still reports `status: not running` *and* a socket path, with exit code 0 — and "not running" contains "running", so a `contains` test is no better than the exit code. Both checks were wrong that way, and the symptom was that a host whose server crashed could never be reconnected to from the app at all: the bootstrap decided a server was already up, forwarded the dead socket, and every attempt failed at the first request, while `herdr --remote <host>` started one fine. `HerdrStatus.isRunning` is the one place that decides (`crashedServerDoesNotReadAsRunning`), the remote bootstrap greps the same line, and it waits for the server it started rather than sleeping — a start that never comes up fails loudly with `/tmp/herdglass-server-$(id -u).log` (per-uid, because a `/tmp` path another user owns is a redirect that fails) instead of handing back a socket path that cannot answer.
...
- **The spaces overlay defers the landing; that is the whole feature.** ⌥⌘↑/↓ and ⌥⌘1…⌥⌘9 both *land* on a space — the host selection moves, the panes being left are parked, the keyboard goes with them and the server is told — so walking six spaces to find one switches six times to arrive once. `SpaceSwitcher` is the same walk with the landing held back until ⌘ comes up (⌘` to step, ⇧⌘` back, or ⌥⌘ held for `holdDuration` to look without stepping), which is what ⌘⇥ does for windows, and it is the only one of the three that shows the list. Three rules keep it honest: the list is **frozen** when the gesture opens, because snapshots keep arriving and an index that moves under a held key commits a space other than the one on screen; committing the space that is already current does **nothing**, since pushing the selection again would take the keyboard off whatever has it; and every arrow belongs to the card, ⌥⌘←/→ and ⌥⌘↑/↓ included, so nothing reaches past it to switch a tab behind it — the monitor swallows them and `step(tabsBy:)`/`step(spacesBy:)` check `isOpen` as well, because a menu item that gets its key first must not act either. The grid itself is rows the window's width decided and then tiles spread evenly over them, so seven spaces in a six-wide window are 4 + 3 rather than a full row with an orphan under it. The arrows move over that grid, ←/→ a tile and ↑/↓ a whole row (a tile, when the tiles fit on one row), and they come off `keyCode`: `unshifted` answers with what the layout types, which for an arrow is the ASCII cursor control and never the `NSUpArrowFunctionKey` spelling a menu's key equivalent is written in, so a character comparison matches nothing and drops every arrow into "not ours", where it took the card down. Its tiles are the sidebar's row vocabulary (`StatusDotView` and `BadgeView` moved next to `StatusStyle` when a second view started drawing them), and the digits answer inside it whether or not Settings turned ⌥⌘1…⌥⌘9 on: the setting buys back nine chords the terminal would not see and nine numbers on a sidebar that is always up, and the overlay spends neither — it draws the number that answers it, while it is up.
- **The overlay is a view over the terminal area, not a floating panel.** A panel would take key window, and the two monitors driving the gesture are `isKeyWindow`-scoped — the release that commits would never arrive. It sits inside the safe area like the strip and the panes (the titlebar is not what it is a shortcut through), and its dim is ghostty's `terminalBackground` at partial alpha rather than a semantic grey, the same rule the unfocused-split scrim follows.
- **A gesture that ends outside the window has to end.** ⌘⇥ away mid-hold is a `flagsChanged` this window never sees, so `windowDidResignKey` cancels the overlay and the pending hold; without it the app comes back with a card up and a keyboard that has moved on.
...
- **Which pane of a split a tab comes back to is this client's to remember.** Herdr's `focused` flag marks the one pane the *server* is in, so every tab the user is not currently looking at reports no focused pane at all — `preferredPane` fell through to `panes.first`, and leaving a split for another tab or space snapped the keyboard and the accent border back to the top-left pane on the way home. `SessionController.lastPaneByTab` records where each tab was left (from `selectPane` and from every settle), `preferredPane` and `settleSelection` prefer it over the server's focus, and it is pruned with the tabs. A pane asking for attention still wins, as it does for tabs.
...
The spaces overlay is a gesture, so it needs the whole of it tried by hand: ⌘`
opens it and steps, held ⌘ with more ` presses keeps stepping and ⇧ goes back,
←/→ move one tile and ↑/↓ a whole row of the grid — narrow the window until the
tiles wrap to check the second — a digit jumps to one of the first nine, and **nothing
switches until ⌘ comes up** — watch the sidebar selection and the pane, which
must not move while the card is up. ⎋ leaves it where it was; a click on a tile
takes it and a click on the dimmed terminal does not. Then hold ⌥⌘ with nothing
else for two seconds: the same card, on the space already showing, and letting go
must change nothing. The cancel is as important as the open — ⌥⌘T, ⌥⌘W and
⌥⌘arrows must never put the card up, however slowly the second key follows. With
one space attached there is nothing to choose between and the card must not
appear at all. ⌘` is macOS's own "next window in this app": it is free here
because there is one window, but it is the kind of combination the system has
been known to take above the app, so it needs a real keyboard and not a
synthesised event.
