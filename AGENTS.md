...
⌥⌘arrows move the keyboard between the panes of a split, which is where
ghostty's own `goto_split` defaults put them. A `keybind` of the user's that
lands on those arrows takes them, and the split item then has no key at all
rather than a key it shares — `super+alt+right=next_tab` in a real config is how
that was found.
...
its row down the *whole* sidebar, hosts included, and the hint the row draws is
the check: press what it says and land on it, with two hosts attached, and see
the hints run 1…9 straight through the host boundary and stop. ⇧⌘1, ⇧⌘2, ⇧⌘7,
⇧⌘8 and ⇧⌘9 are the ones a stock Mac lets through.
